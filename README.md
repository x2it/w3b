...
| 静态资源缓存版本 | **v3.3.8**（CSS / JS 引用的 `?v=x.y.z`，用于强制 CDN 刷新；v3.3.8 修复订阅弹窗 JS 事件缺失） |
...
- **v3.3.0 响应式高度体系**（`assets/js/extras-index.js` + `style.css` 双重兜底）：
  ...
  - **20000ms（20s）超时兜底**：若上游未发 postMessage，自动使用当前断点的最终高度 = `default + bias`（博客 D=1000+75=**1075**、M=1200-100=**1100**、S=1250-100=**1150**；7喵 D=780、M=950、S=980；bias=0）。收到 postMessage 后先按自然 `[min, max]` 夹紧再叠加 `bias`，确保用户每次「±调参」都有真实像素变化（即使 raw 在自然区间内）。
...
4. Cloudflare CDN 缓存约 5-10 分钟刷新；修改 CSS / JS 后务必同步更新 `index.html` / `about.html` / `404.html` 中的 `?v=x.y.z` 版本号（三页保持一致，当前 `?v=3.3.8`）
...
- **每次修改 CSS / JS（哪怕 1 行）必须同步 bump 3 个页面的 `?v=` 查询参数**：
  ...
  - 当前 3 页：`index.html` / `about.html` / `404.html` 必须完全一致（目前为 `?v=3.3.8`）
